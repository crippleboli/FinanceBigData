...
print(f'最佳窗口大小下的MLP Accuracy: {best_accuracy}')
print(f'最佳窗口大小下的MLP ROC AUC: {best_roc_auc}')
...
# 特征重要性（对于神经网络来说，不太直观，因此这里显示每个特征的平均权重绝对值）
coefs = clf.coefs_[0]
importances = np.mean(np.abs(coefs), axis=1)
indices = np.argsort(importances)[::-1]

plt.figure(figsize=(15, 5))
plt.title("Feature Importances (Mean Absolute Weights)")
